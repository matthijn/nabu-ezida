...
export const detectHint = (ctx: HintContext): string | null => {
  ...
  return (
    detectWholeBlockHint(classified) ??
...
const detectJsonStructureHint = (classified: ClassifiedHunk[]): string | null => {
  const hasStructureEdit = classified.some(
    (c) => !isPureAppendClassified(c) && touchesStructureOrMixed(c)
  )
...
const detectLargeHunkHint = (classified: ClassifiedHunk[]): string | null => {
  const hasLargeHunk = classified.some(
    (c) => c.addLineCount > LARGE_HUNK_THRESHOLD && !c.isRangeRef
  )
