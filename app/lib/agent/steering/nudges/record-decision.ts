...
const hasUnrecordedAsk = (history: Block[]): boolean => {
  for (let i = history.length - 1; i >= 0; i--) {
    ...
    if (block.type === "tool_result" && (block as { toolName?: string }).toolName === "record_decision") return false
...
const baseNudge: Nudger = (history) =>
  hasUnrecordedAsk(history) ? systemNudge(prompt) : null
