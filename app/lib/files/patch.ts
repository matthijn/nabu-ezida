...
const applyMdPatch = (path: string, content: string, patch: string): FileResult => {
  ...
  const { result: patchWithIds, generated: placeholderIds } = replaceUuidPlaceholders(patch)
  ...
  const validation = validateMarkdownBlocks(filledContent, { context, original: content })
  ...
  const generatedIds = buildGeneratedIdsList(placeholderIds, autoGenerated, filledContent)
