...
import { derive, hasCall, type DerivedPlan, type DerivedExploration } from "~/lib/agent"
...
const findCreationIndices = (history: Block[], toolName: string): number[] =>
  history
    .map((b, i) => ({ block: b, index: i }))
    .filter(({ block }) => hasCall(block, toolName))
    .map(({ index }) => index)
...
const explorationMessagesIndexed = (
  ...
  return [{ index, message: { type: "exploration", exploration, completed: exploration.completed } }]
}
