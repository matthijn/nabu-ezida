...
const parseBlock = (markdown: string, language = "json-annotations"): unknown => {
...
describe("stampActors", () => {
  describe("array-level (*.actor in json-annotations)", () => {
    const cases = [
      {
        name: "stamps actor on new annotations (no original)",
        original: "",
        updated: md(annotations([{ id: "a_1", text: "hello", color: "blue" }])),
        actor: "ai" as const,
        expected: (result: string) => {
          const anns = parseBlock(result) as Record<string, unknown>[]
          expect(anns[0].actor).toBe("ai")
        },
      },
      {
        name: "stamps actor on new annotation (has original without this id)",
        original: md(annotations([{ id: "a_1", text: "existing", color: "red", actor: "user" }])),
        updated: md(
          annotations([
      ...
      },
      {
        ...
        original: md(annotations([{ id: "a_1", text: "hello", color: "blue", actor: "user" }])),
        updated: md(annotations([{ id: "a_1", text: "hello", color: "blue", actor: "user" }])),
        actor: "ai" as const,
        expected: (result: string) => {
          const anns = parseBlock(result) as Record<string, unknown>[]
          expect(anns[0].actor).toBe("user")
      ...
      },
      {
        ...
        original: md(annotations([{ id: "a_1", text: "hello", color: "blue", actor: "user" }])),
        updated: md(annotations([{ id: "a_1", text: "hello", color: "red", actor: "user" }])),
        actor: "ai" as const,
        expected: (result: string) => {
          const anns = parseBlock(result) as Record<string, unknown>[]
          expect(anns[0].actor).toBe("ai")
      ...
      },
      {
        ...
        original: md(annotations([{ id: "a_1", text: "hello", color: "blue", actor: "ai" }])),
        updated: md(annotations([{ id: "a_1", text: "changed", color: "blue", actor: "user" }])),
        actor: "ai" as const,
        expected: (result: string) => {
          const anns = parseBlock(result) as Record<string, unknown>[]
          expect(anns[0].actor).toBe("ai")
    ...
    const cases = [
      {
        ...
        actor: "ai" as const,
        expected: (result: string) => {
          const parsed = parseBlock(result, "json-callout") as Record<string, unknown>
